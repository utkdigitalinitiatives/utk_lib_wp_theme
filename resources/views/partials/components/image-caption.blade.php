    ...
    @if( ! empty( isset($image['caption']) && ! empty( $image['caption'] ) && $captionEnabled === true ) )
        <div class="utk-image--caption">
            ...
            @if( ! empty($captionLink) )
                <a href="@php echo $captionLink; @endphp"><strong>More</strong><span class="sr-only"> about image of @php echo $image['caption'] @endphp</span><span class="icon-right-big"></span></a>
            @endif
        </div>
    @endif
@endif